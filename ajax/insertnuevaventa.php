...
date_default_timezone_set('America/Lima');
		if (empty($_POST['txtid'])  || empty($_POST['tipomovi']) || empty($_POST['txtidpro']) || empty($_POST['txtin']) ||  empty($_POST['txttot']) || empty($_POST['txtiddoc']) || empty($_POST['area']) ){ 
			$errors[] = "Verifique Datos";
		}  elseif (
			!empty($_POST['txtid']) && !empty($_POST['tipomovi']) && !empty($_POST['txtidpro']) && !empty($_POST['txtin']) && !empty($_POST['txttot']) && !empty($_POST['txtiddoc']) && !empty($_POST['area'])
        ) {
			...
			//echo "<pre>";
				...
				$doctor = strip_tags($_POST["txtiddoc"],ENT_QUOTES); 
                                
                                $idarea = strip_tags($_POST["area"],ENT_QUOTES); 
				...
				$registro=$_SESSION['user_id'];
                                ...
                                if ($tcomprobate ==1) {
                                 ...
                                 $sql = "INSERT INTO documento (comprobante,tipopago,fecha,total,"
                                  . "acuenta,id_movimiento,porcentajeigv,id_cliente,id_usuario,iddoctor,idarea) "
                                  . "VALUES ('Boleta', 'Contado', '".$Fecharegistro."', '".$to."', NULL, '".$timo."', '".$igv."', '".$id."', '".$registro."','".$doctor."','".$idarea."');";  
                            ...
                            // echo $sql;
                                  ...
                                  $stock="SELECT stock,factor FROM producto where id_producto='".$pro."'";
                                  $ecu= mysqli_query($conexion, $stock);
                                  $fi=mysqli_fetch_assoc($ecu);
                                  $st_ac=$fi['stock'];
                                  $stockActulizado=$st_ac-(($_POST['txtcan'][$key])/($fi['factor']));
				...
				//$dir = strip_tags($_POST["txtdir"],ENT_QUOTES); 
                                 ...
                                 $sql = "INSERT INTO documento (comprobante,tipopago,fecha,total,"
                                  . "acuenta,id_movimiento,porcentajeigv,id_cliente,ruc,razonsocial,direccion,id_usuario,iddoctor) "
                                  . "VALUES ('Factura', 'Contado', '".$Fecharegistro."', '".$to."', NULL, '".$timo."', '".$igv."', '".$id."', 'NULL', 'NULL', 'NULL', '".$registro."','".$doctor."');";  
                                  $query_new_boleta_insert = mysqli_query($conexion,$sql);
                                   $last_id = $conexion->insert_id;
