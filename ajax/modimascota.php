...
date_default_timezone_set('America/Lima');
	 ...
	 !empty($_POST['txtid']) && !empty($_POST['txtnom']) && !empty($_POST['txtes']) && !empty($_POST['txtra']) && !empty($_POST['txtse']) && !empty($_POST['txtpe']) && !empty($_POST['txtfn']) && !empty($_POST['txtco']) 
			...
			require_once ("../config/conexion.php");
				...
				$extra = strip_tags($_POST["txtex"],ENT_QUOTES); 
                               $señas = strip_tags($_POST["txtseñas"],ENT_QUOTES); 
				...
				//$registro=$_SESSION['user_id'];
                                if(empty($_FILES['txtfoto']['name'])){
                                    $sql = "UPDATE mascota  set nombre='".$nom."', especie='".$es."',raza='".$ra."',sexo='".$se."',pelaje='".$pe."',f_nac='".$fn."',fallecido='".$fa."',extraviado='".$extra."',señas='".$señas."',codigodvi='".$dvi."' where id_mascota=$idc";
	...
	$limite_kb = 16384;
          if (in_array($_FILES['txtfoto']['type'], $permitidos) && $_FILES['txtfoto']['size'] <= $limite_kb * 1024){       
                    $sql = "UPDATE mascota  set nombre='".$nom."', especie='".$es."',raza='".$ra."',sexo='".$se."',pelaje='".$pe."',f_nac='".$fn."',fallecido='".$fa."',extraviado='".$extra."',foto='".$fotob64."',señas='".$señas."',codigodvi='".$dvi."' where id_mascota=$idc";
                  // echo $sql; 
                    $query_new_apoderado_insert = mysqli_query($conexion,$sql);
            }
                                }
