...
date_default_timezone_set('America/Lima');
		...
		if ( empty($_POST['txtid']) || empty($_POST['txtape']) || empty($_POST['txtnom']) || empty($_POST['txtdomi'])   ){ 
			$errors[] = "Verifique Datos";
		}  elseif (
			 !empty($_POST['txtid']) && !empty($_POST['txtape']) && !empty($_POST['txtnom']) && !empty($_POST['txtdomi']) 
        ) {
            ...
            require_once ("../config/conexion.php");//Contiene funcion que conecta a la base de datos
				...
				//$registro=$_SESSION['user_id'];
                 ...
                 $sql = "UPDATE  doctor SET nombre ='".$nom."', apellidos='".$ape."',direccion='".$domi."',celular='".$t1."',profesion='".$pro."',dni='".$dni."',email='".$ema."'  where iddoctor ='".$id."'";
